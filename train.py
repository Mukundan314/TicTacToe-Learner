...
if os.path.exists('states.npy'):
    states = states.load_states()
else:
    states = np.unique(states.generate_states(), axis=0)
    states.save_states(states)
...
for i in tqdm.trange(100000, desc="Training"):
    ...
    for i, x_move in enumerate(x_moves):
        ...
        if state == 3:
            q_mat[x_move[1]] = change_prob(q_mat[x_move[1]], x_move[0],
                                           (i + 1) / 2)
    ...
    for i, o_move in enumerate(o_moves):
        ...
        if state == 3:
            q_mat[o_move[1]] = change_prob(q_mat[o_move[1]], o_move[0],
                                           (i + 1) / 2)
